...
bool sqlOperation::Init(const QString & host, const QString & user, const QString & passwd, const QString & dataname,const QString & type)
{
...
//清空数据库现存数据
bool sqlOperation::clearTable(QStringList listTableName, QSqlQuery ** ret)
{
    
    for (int i = 0; i < listTableName.size(); i++) {
        QString sql = QString("truncate table %1")
            .arg(listTableName.at(i));
        m_query->prepare(sql);
        if (!Query(sql, ret)) {
            return false;
        }
