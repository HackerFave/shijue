...
{
    checked = false;
    buttonStyle = ButtonStyle_CircleIn;// ButtonStyle_Image;

    bgColorOff = QColor(60, 60, 60);
    bgColorOn = "#00aaff";
    ...
    imageOff = tr("%1/icons/switchoff.png").arg(publicClass::instance()->getAppPath());
    imageOn =  tr("%1/icons/switchon.png").arg(publicClass::instance()->getAppPath());
...
void SwitchButton::drawBg(QPainter *painter)
{
    ...
    if (buttonStyle == ButtonStyle_Rect) {
        ...
        QRect rect(space, space, width() - space * 4, height() - space * 4);
        painter->drawRoundedRect(rect, rectRadius, rectRadius);
    }
...
void SwitchButton::drawImage(QPainter *painter)
{
    ...
    //QPixmap scaledPixmap = pix.scaled(pix->getCursorSize() + 2, btnDrog->getCursorSize() + 2, Qt::KeepAspectRatio);
    //自动等比例平滑缩放居中显示
    int targetWidth = pix.width()/2;
    int targetHeight = pix.height()/2;
